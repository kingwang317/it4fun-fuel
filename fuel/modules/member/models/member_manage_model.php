...
class Member_manage_model extends MY_Model {
	...
	public function do_edit_member($member_id, $member_account, $member_name, $member_mobile, $member_addr, $vat_num, $inv_title)
	{
		$sql = @"UPDATE mod_member SET member_account=?,member_name=?,member_mobile=?,member_addr=?,vat_number=?, invoice_title=?, modi_time=NOW() WHERE member_id=?";

		$para = array($member_account, $member_name, $member_mobile, $member_addr, $vat_num, $inv_title, $member_id);
		$success = $this->db->query($sql, $para);
	...
	{
		$sql = @"SELECT cli_id, cli_title, cli_fbid FROM mod_client WHERE cli_fbid=? LIMIT 0,1";
		$para = array($fbid);
		$query = $this->db->query($sql, $para);
